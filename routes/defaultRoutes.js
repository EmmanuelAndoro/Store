...
const {index, about, contact, news, farmingpractice, ourproduct,registerGet, 
      registerPost,storeVerify,loginGet,loginPost} = require('../controllers/defaultControllers');
...
router.get("/farmingpractice", farmingpractice);

//our product Route
router.get("/ourproduct", ourproduct);
...
passport.use(new localStrategy({
    ...
    await Store.findOne({email: email}).then(async(store)=>{ //in passing one parameter one can remove the paranthesis
        ...
        await bcrypt.compare(password, store.password, (err, passwordMatched, )=>{
            ...
            return done(null, false, req.flash('success-message', 'login Successfull!!!')); //add change false to store
...
console.log('is store') // checking to see if it going to the store
